...
unsafe fn error_check(nlhs: i32, nrhs: i32, prhs: *const *const raw::mxArray){
    ...
    if nrhs != 1 || raw::mxIsNumeric(*prhs.add(0)) == false || raw::mxGetNumberOfElements(*prhs.add(0)) != 1{
        raw::mexErrMsgIdAndTxt(b"MATLAB:arraySize:rhs\0".as_ptr().cast(),
                b"This function requires one scalar numeric input.\0".as_ptr().cast());
    }
    let dim = raw::mxGetScalar(*prhs.add(0));
    if dim < 0. {
        raw::mexErrMsgIdAndTxt(b"MATLAB:arraySize:dimensionNegative\0".as_ptr().cast(),
    ...
    if nlhs > 1{
        raw::mexErrMsgIdAndTxt(b"MATLAB:arraySize:lhs\0".as_ptr().cast(),b"Too many output arguments.\0".as_ptr().cast());
    }
}
...
pub unsafe extern "C" fn mexFunction(
    ...
    let the_array = raw::mxCreateNumericMatrix(dim as raw::mwSize, dim as raw::mwSize, raw::mxClassID_mxUINT8_CLASS, raw::mxComplexity_mxREAL);
    ...
    let size_of_data_in_kibibytes = number_of_elements as f64 * raw::mxGetElementSize(the_array) as f64 / 1024.;
    raw::mexPrintf(b"Size of array in kilobytes: %.0f\n\n\0".as_ptr().cast(), size_of_data_in_kibibytes);
